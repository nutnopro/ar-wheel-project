...
const ManageCategoriesScreen = () => {
  ...
  const handleDelete = (id: string) => {
    Alert.alert("Confirm", "Delete category?", [{ text: "Cancel" }, { text: "Delete", style: 'destructive', onPress: () => setCats(prev => prev.map(c => c.id === id ? { ...c, status: 'Deleted' } : c)) }]);
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.background }}>
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[styles.card, { backgroundColor: theme.card, opacity: isDeleted ? 0.6 : 1 }]}>
              <View style={[styles.iconBox, { backgroundColor: '#F3E8FF' }]}><Icon name="shape" size={24} color="#8B5CF6" /></View>
              <View style={{ flex: 1 }}>
              ...
              <View style={styles.actions}>
                <TouchableOpacity onPress={() => Alert.alert('Edit', item.name)} disabled={isDeleted} style={styles.actionBtn}><Icon name="pencil-outline" size={24} color={isDeleted ? theme.subText : "#F59E0B"} /></TouchableOpacity>
      ...
      <View style={[styles.footer, { backgroundColor: theme.card, borderTopColor: theme.border }]}>
        <TouchableOpacity style={[styles.addButton, { backgroundColor: '#8B5CF6' }]} onPress={() => Alert.alert('Add', 'Category')}><Icon name="plus" size={24} color="#fff" /><Text style={styles.addText}>Add Category</Text></TouchableOpacity>
      </View>
    </View>
  );
};
