...
class ARUIManager(private val context: Context, private val rootLayout: FrameLayout, private val overlayView: View) {
    ...
    // *Mock Data (รอรับจาก React Native)
    ...
    fun setupInterface() {
        ...
        // *Bottom Bar (Model, Capture, Settings)
        setupBottomBar()

        // *Selection Menu Container (Hidden by default)
    ...
    private fun setupTopBar() {
        ...
        topContainer.addView(btnBack)
        topContainer.addView(createSpacer(30))
        topContainer.addView(btnModeToggle)
    ...
    private fun setupDebugPanel() {
        ...
        debugContainer.addView(overlayToggle)
    ...
    private fun setupBottomBar() {
        ...
        bottomContainer.addView(leftLayout)
        bottomContainer.addView(centerLayout)
        bottomContainer.addView(rightLayout)

        rootLayout.addView(bottomContainer)
    }

    private fun setupSelectionContainer() {
    ...
    private fun updateModeIcon() {
        ...
        val text = if (currentMode == ARMode.MARKER_BASED) "Scan" else "Marker"
        btnModeToggle?.text = "$icon $text"
    }
    ...
    // *Function to update models from Activity/React Native
...
//             orientation = LinearLayout.HORIZONTAL
//             gravity = Gravity.CENTER
//             // Use weight sum for spacing distribution
//             weightSum = 3f 
//             layoutParams = FrameLayout.LayoutParams(
//                 FrameLayout.LayoutParams.MATCH_PARENT,
//                 FrameLayout.LayoutParams.WRAP_CONTENT
...
//         // Add to Layout using Wrappers for Weight/Gravity control
//         // Note: Views are added to list for easier rotation later
