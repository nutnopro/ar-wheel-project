...
class ModelManager(private val arSceneView: ARSceneView) {
    ...
    fun createNewModel(modelPath: String): Node {
        ...
        modelLoader.createModelInstance(assetFileLocation = modelPath)?.let { modelInstance ->
            setupModelNode(rootNode, ModelNode(modelInstance = modelInstance))
        }

        return rootNode
    }

    private fun setupModelNode(rootNode: Node, modelNode: ModelNode) {
        ...
        modelNode.position = Float3(0f, 0f, -halfZ + 0.1f)

        addTubeAndBackplate(rootNode, maxDimension, radius, sizeZ, halfZ)
        rootNode.addChildNode(modelNode)
    }

    private fun createBackplate(radius: Float): CylinderNode {
    ...
    private fun createTube(maxDimension: Float, sizeZ: Float): ModelNode {
    ...
    private fun addTubeAndBackplate(rootNode: Node, maxDimension: Float, radius: Float, sizeZ: Float, halfZ: Float) {
        val backplate = createBackplate(radius).apply { position = Float3(0f, 0f, sizeZ) }
        val tube = createTube(maxDimension, sizeZ).apply { position = Float3(0f, 0f, -halfZ + 0.1f) }
