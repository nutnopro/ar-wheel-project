...
class ModelManager(private val arSceneView: ARSceneView) {
	...
	fun createModelNode(modelPath: String): ModelNode {
		val rootNode = ModelNode().apply { isVisible = false }
		...
		val size = boundingBox?.size ?: Float3(1f, 1f, 1f)
        ...
        model.addChildNode(model)
        model.addChildNode(backplate)

		return rootNode
	}

	fun createBackplate(radius: Float): CylinderNode {
		val blackMaterial = loader.materialLoader.createColorInstance(
